...
export class VersionService {
  ...
  createVersion(name: string, image: string, replicas: number, volumes: string, environmentId: number, environmentUuid: string): Observable<Version> {
    const options = new RequestOptions({ headers: this.getHeaders() });
    const data = { name: name, image: image, replicas: replicas, volumes: volumes, environment_id: environmentId }
    console.log(data);
